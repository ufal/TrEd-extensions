...
<xsl:stylesheet
    ...
    <!-- An index for fast lookup of terminals nad nonterminals by their id. -->
    ...
    <xsl:template match="t">
        ...
        <terminal>
            <xsl:apply-templates select="@*"/>
            <order><xsl:value-of select="count(preceding-sibling::t)+1"/></order>
            <xsl:if test="$label!=''">
            ...
            <children>
                <xsl:for-each select="$children">
                    <!-- we order terminals by their document order
                         ...
                         Note that following selector is ugly; however, best that can be done without recursion in XPath.
                    -->
                    <xsl:sort select="count(
                        self::t/preceding-sibling::t
                        |
                        key('id', @idref)/self::t/preceding-sibling::t
                        |
                        (key('id',
                            key('id',@idref)/self::nt/edge/@idref
                            |
                        ...
                        )/self::t)[1]/preceding-sibling::t
                    )" data-type="number" order="ascending" />

                    <!-- Apply a template to correctly process either current edge, or an orphant terminal. -->
                    <xsl:if test="self::t">
                        <xsl:apply-templates select="self::t">
                            <!-- Orphan terminals are attached with specific label '$'. -->
                            <xsl:with-param name="label">$</xsl:with-param>
                        </xsl:apply-templates>
                    </xsl:if>
                    <xsl:if test="self::edge">
                        <xsl:apply-templates select="self::edge" />
                    </xsl:if>
                </xsl:for-each>
            </children>
        </xsl:if>
    ...
    <xsl:template match="nt">
        ...
        <nonterminal>
            ...
            <!-- Process Children -->
            <xsl:if test="@id = ancestor::graph/@root">
                <!-- Treat the root nonterminal differently as it must also contain orphan terminals. -->
                <xsl:call-template name="nt_children">
                    <xsl:with-param name="children" select="edge|ancestor::graph//t[not(@id = ancestor::graph//edge/@idref)]" />
                </xsl:call-template>
            </xsl:if>
            <xsl:if test="@id != ancestor::graph/@root">
                <!-- Nested nonterminals are processed by standard DFS (thus we include only child edges). -->
                <xsl:call-template name="nt_children">
                    <xsl:with-param name="children" select="edge" />
                </xsl:call-template>
            </xsl:if>
        </nonterminal>
    </xsl:template>
